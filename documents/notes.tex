...
\begin{figure}[H]
    ...
    \label{fig:my_label}
\end{figure} 
\subsection{Creazione di una Datapipeline con Apache Kafka, Apache Druid e Docker Compose}
Ora dopo aver fatto alcune sperimentazioni con \textbf{Apache Kafka}, \textbf{Apache Druid} e aver costruito un generatore di eventi, riutilizziamo tali componenti per creiamo una pipeline dati in grado di generare 5000000 di dati ed effettuare il \textbf{Data Injection} all'interno di \textbf{Druid}.
\subsubsection{Svolgimento}
\begin{itemize}
    \item Come primo passo è stato creato un generatore di eventi in \textbf{Python}, utilizzando il modulo \textbf{Faker}. Tali eventi verranno pubblicati poi in Kafka, all'interno del topic \textbf{registrazione}. Il modello relazionale dei dati è il seguente (i tipi riportati sono affini al DBMS \textbf{PostgreSQL}).
    
\begin{lstlisting}[language=SQL]
CREATE TABLE registrazione(nome text, cognome text, indirizzo text,
citta text, stato text, cap int, email text, telefono text, eta int,
altezza decimal(5,2), peso decimal(5,2), reddito decimal(6,2),
datan date, professione text, istruzione text, hobby text, nfigli int,
codice_cliente int, datareg timestamp, ultimoacc timestamp);

\end{lstlisting}
    \item Il generatore di eventi utilizzerà una randomicità ristretta sui dati generati per avere un maggiore controllo su questi ultimi
    \begin{lstlisting}[language=Python]
from time import sleep  
from kafka import KafkaProducer  
import json 
