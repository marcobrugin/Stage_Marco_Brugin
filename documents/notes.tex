...
\subsubsection{Injection dei dati}
 ...
 \begin{itemize}
     \item Metodi di ingestione "seekable-stream" supervisionati come Kafka e Kinesis . Con questi metodi, Druid esegue il commit degli offset del flusso nel proprio archivio di metadati insieme ai metadati del segmento, nella stessa transazione, in qyesti casi è possibile andare ad effettuare operazioni di {rollup}\ref{sub} sui dati grezzi;
...
\centering
 & \centering Apache Druid [s] & \centering PostgreSQL [s] \\\hline 
  \centering
  query1 &  \centering  0.2 & \centering 1.2 \\ \hline
  \centering
  query2 &  \centering 0.1 &  \centering 1.9 \\ \hline 
  \centering
  query3 & \centering 0.4 & \centering 2 \\ \hline
  \centering  query4 & \centering 0.2 & \centering 1.1 \\ \hline
  \end{tblr}
\end{center}
\subsubsection{Conclusioni}
Dalla seguente sperimentazione si evince una chiara superiorità di \textbf{Apache Druid} di un ordine di grandezza su un comune database relazione come \textbf{PostgreSQL}, in particolare nell'esecuzione di query che coninvolgono raggrupamenti, anche su intervalli temporali (ora, giorno, anno).
