...
\title{ \normalsize \textsc{}
		...
		\HRule{2.0pt} \\ [0.6cm] \LARGE{In questo documento vengono riportati i concetti affrontati durante lo stage presso Sync Lab s.r.L.} \vspace*{10\baselineskip}}
		}
\date{}
\author{\textbf{Author} \\ 
		Marco Brugin \\
...
Esistono molti client per diversi linguaggi di programmazione.
...
Gli uni  sono indipendenti l'uno dall'altro, ciò permette di raggiungere alcune delle seguenti garanzie:
\begin{itemize}
    \item \textbf{Al massimo una volta}: i messaggi possono andare persi ma mai riconsegnati. Infatti si potrebbe verificare un guasto ad un singolo broker o un errore della rete nel momento dell'invio di un messaggio. Il messaggio stesso verrà recapitato una e una sola volta.
    \item \textbf{Almeno una volta}: i messaggi non 		vengono mai persi ma possono essere 		riconsegnati;
...
Ciò aiuta a mantenere alti livelli di disponibilità in caso di guasto. \\Le repliche sono permesse  a livello di partizione. \textbf{Kafka} ne designa una chimata leader mentre le altre  sono partizioni follower o \textbf{in-sync}. Il numero totale di repliche incluso il leader costituisce il fattore di replicazione.
Il leader è responsabile della ricezione e dell'invio dei dati, per quella partizione.
Per mantenere questi cluster e gli argomenti/partizioni all'interno, \textbf{Kafka} ha un servizio centralizzato chiamato \textbf{Zookeeper}.
...
I parametri configurabili per la    \textbf{Retention} basata sul tempo sono in ordine di importanza e valutazione:
\begin{enumerate}
...
\\
I parametri configurabili per la   \textbf{Retention} basata dimensione sono in ordine di importanza e valutazione:
\begin{enumerate}
...
\textbf{Kafka} è un sistema di streaming di eventi che gestisce un flusso continuo di eventi. Inoltre \textbf{Kafka} memorizza tali in modo duraturo per il successivo recupero, analisi o elaborazione in tempo reale e li inoltri a varie destinazioni secondo necessità.
\\
...
È un modello architetturale basato su più livelli orrizontali, in cui ciascuno svolge un compito specifico e ha determinata responsabilità. Sebbene il pattern in sè non indichi il numero di livelli da utilizzare, è usuale creare architetture con al più quattro livelli: presentazione, business, persistenza e database.
...
In generale la comunicazione avviene attraverso livelli chiusi: il passaggio delle informazione tra due o più livelli deve attraversare tutti i livelli intermedi.
...
\\
In generale le pipeline dati tradizionali estraggono, trasformano e caricano i dati prima che possano essere utilizzati.
...
\begin{itemize}
    ...
    \item 
    \textbf{Tolleranza ai guasti}: lo streaming dei dati non smettono mai di fluire da varie sorgenti e il sistema deve prevenire le interuzioni di alcuni sorgenti;
    \item 
...
Il concetto di alta affidabilità è essenzialmente legato alla capacità di un sistema di funzionare continuamente senza guasti in un determinato periodo di tempo.\\
...
\begin{itemize}
    ...
    il carico di elaborazione su tutte le risorse del sistema in modo tale che nessuna sia sovraccaricata;
...
Con il termine \textbf{distribuito} si va ad intendere un'architettura nella quale i componenti non si trovano su un'unica macchina ma possono cooperare attraverso una rete di comunicazione per raggiungere un determinato obiettivo.
...
\begin{itemize}
    ...
    \item il \textbf{Middleware} è un'infrastruttura che supporta adeguatamente lo sviluppo e l'esecuzione di applicazioni distribuite, fornendo un buffer tra le applicazioni e la rete. Svolge il ruolo di intermediario del sistema e gestisce o supporta i diversi componenti di un sistema distribuito;
...
\textbf{Apache Druid}  è uno strumento di analisi \textbf{OLAP} progettato per analisi rapide, i suoi punti di forza sono le alte prestazioni in esecuzione di query su set di dati di grandi dimensioni.
\textbf{Druid} è comunemente utilizzato come back-end  per le GUI di applicazioni analitiche o per API altamente simultanee che richiedono aggregazioni veloci. Mostra le sue migliori prestazioni con dati orientati agli eventi.
...
\begin{itemize}
    ...
    \item l'architettura nativa del cloud è tollerante ai guasti: \textbf{Druid} archivia in modo sicuro una copia dei tuoi dati in un archivio profondo (in genere su su file system o sul cloud), tali dati saranno accessibili semopre anche nel momento in cui i server falliscano; ciò permette l'esecuzione di query anche nei momenti di ripristino.
...
Infine unisce  tutti i risultati elaborati per fornire un'unica risposta alla query richiesta.
\subsubsection{Servizio di router (opzionale)}
 ...
 Ogni servizio  copia o estrae i file di segmento da Deep Storage al disco locale in un'area chiamata \textit{segment cache} e rispondono alle domande su tali segmenti.\\Il coordinatore controlla l'assegnazione dei segmenti agli storici e l'equilibrio dei segmenti tra gli storici.  I servizi storici non comunicano direttamente tra loro, né comunicano direttamente con il Coordinatore. Invece, il coordinatore crea voci temporanee in \textbf{Zookeeper} in un percorso della coda di caricamento. Ogni processo storico mantiene una connessione con \textbf{Zookeeper}, osservando quei percorsi per le informazioni sui segmenti.
\subsubsection{Servizio MiddleManager}
Il processo MiddleManager gestisce l'inserimento di nuovi dati nel cluster. È responsabile della lettura da fonti di dati esterne e della pubblicazione di nuovi segmenti Druid.
...
In alternativa ai \textbf{MiddleManger} ed ai \textbf{Peon}.\\
...
Il \textbf{deep storage} è il sistema nel quale vengono archiviati i segmenti. È un mceccanismo non fornito da \textbf{Druid}.\\
...
Hanno lo scopo di memorizzare  gli oggetti di configurazione di runtime.\\
...
\begin{itemize}
    ...
    \item protocollo di caricamento/rilascio del segmento tra Coordinator e Historical;
...
\textbf{Druid} crea un segmento per ogni intervallo di tempo che contiene dati.
...
\begin{itemize}
    ...
    \item disponibilità per l'interrogazione: i segmenti sono disponibili per l'interrogazione su alcuni server di dati \textbf{Druid}, come un'attività in tempo reale o un processo storico.
\end{itemize}

\subsubsection{Injection dei dati}
 I principali metodi di injection  di Druid sono tutti basati su pull e offrono garanzie transazionali (verrà caricato o tutto o niente).
 \begin{itemize}
...
\begin{itemize}
 ...
 \item ogni riga in \textbf{Druid} deve avere un timestamp, tale informazione potrà essere utilizzata per successivi grappamenti;
 \item tutte le colonne del modello di archiviazione di     \textbf{Druid} sono dimension o metriche ad eccezione del timestamp;
...
È possibile fare in modo che \textbf{Druid} deduca lo schema e i tipi per i tuoi dati, in modo automatico o attraverso l'intervento umano.\\
...
Da questo punto di vista il \textbf{rollup} non è altro che una forma di riepilogo e preaggregamento dei dati finalizzato a ridurre l'ordine di grandezza delle righe da archiviare.\\
...
È convenienete andare ad utilizzare il \textbf{rollup} se si desiderano prestazioni ottimali e vicoli di spazio rigorosi o quando non si ha necessità di avere a disposizione dati grezzi con alta cardinalità. \\
...
Mentre le caratteristiche che garantiscono il \textbf{Best-effort rollup} lo fanno per i seguenti motivi: 
\begin{itemize}
...
I nodi \textbf{worker} ospitano i \textbf{POD}, che rappresentano le minime unità di calcolo, distribuibili e gestibili su \textbf{Kubernetes}. Il \textbf{control pane} gestisce i nodi di lavoro e i \textbf{POD} del cluster. In ambienti di produzione per garantire tolleranza ai guasti e alta disponibilità il \textbf{control pane} e \textbf{POD} sono eseguiti su più calcolatori e un cluster esegue i nodi. 
...
\begin{itemize}
    \item  aggiorna il nodo $.status$ quando c'è un cambiamento di stato o se non c'è stato alcun aggiornamento per un intervallo configurato (l'intervallo predefinito è di 5 minuti);
    \item Il \textbf{kubelet} crea e quindi aggiorna il suo oggetto \textbf{Lease} ogni 10 secondi.
\end{itemize}
...
\end{itemize} 
\subsection{I service}
In \textbf{Kubernetes}, un servizio è un metodo per esporre un'applicazione di rete in esecuzione come una o più \textbf{PODS}. 
\subsection{Il ConfigMap}
Un \textbf{ConfigMap} è un oggetto API utilizzato per archiviare dati, possono essere utilizzate come variabili d'ambiente.
\subsection{Configurazione con minikube}
...
\includegraphics{images/output_pods.png}
    ...
    \end{lstlisting}
      \begin{figure}[h]
        \centering
\includegraphics[width=0.5\textwidth]{images/get_events.png}
        \caption{Get events}
    ...
    \end{lstlisting}
     \begin{figure}[H]
        \centering
\includegraphics[width=0.5\textwidth]{images/get_config_view.png}
        \caption{Config view}
        \label{fig:config_view}
    \end{figure}
\end{enumerate}
Assegnare un \textbf{PODS} a un nodo.
\begin{enumerate}
    \item Creare un POD;
    \begin{lstlisting}
apiVersion: v1
...
spec:
  ...
  - name: nome-del-contenitore
    ...
    \end{lstlisting}
    \item elencare i nodi del nodi del cluster;
    \begin{lstlisting}
        kubectl get nodes --show-labels
    \end{lstlisting}
    \item scegliere un nodo;
    \begin{lstlisting}
    ...
    \end{lstlisting}
\end{enumerate}
\subsection{Configurazione attraverso file kubeconfig}
Il file \textbf{kubeconfig} viene utilizzato per immagazzinare le informazioni di autenticazione per \textbf{kubectl}. La posizione predefinita del \textbf{kubeconfig} file è  $~/.kube$. \\ In generale invece di utilizzare il nome \textbf{kubeconfig}, il file viene semplicemente denomicato \textbf{config}.
\subsubsection{Composizione}
il \textbf{kubeconfig} file è un file YAML contenente tutti i gruppi di \textbf{cluster}, di \textbf{utenti} e di \textbf{contesti}.\\
\begin{itemize}
    \item Un \textbf{cluster} è un cluster \textbf{Kubernetes};
    \item un utente è una credenziale utilizzata per interagire con \textbf{API Kubernetes};
    \item un \textbf{contesto} è una combinazione di un cluster e un utente, ogni volta che si esegue il comando \textbf{kubectl} si fa riferimento ad un contesto presente all'interno del file \textbf{kubeconfig}.
\end{itemize}
\subsection{Esempio di deployment di minikube}
\begin{lstlisting}
apiVersion: v1
kind: Config
clusters:
...
\end{lstlisting}
\begin{itemize}
    \item La sezione cluster elenca tutti i cluster che sono già stati connessi. In questo caso, ce solo minikube (il nome è arbitrario). All'interno del \textbf{cluster} sono presenti due chiavi:
    \begin{itemize}
        \item \textbf{certificate-authority}: contiene il certificato per l'autorità di certificazione. Può trattarsi di un certificato di un percorso file o di una stringa Base64 del formato PEM (Privacy Enhanced Mail) del certificato;
        \item \textbf{server}: è l'indirizzo del server;
    \end{itemize}
    \item la sezione utenti elenca tutti gli utenti già utilizzati per connettersi a un cluster. Esistono alcune possibili chiavi per un utente:
    \begin{itemize}
        \item \textbf{client-certificate}: contiene un certificato per l'utente firmato dalla CA Kubernetes. È un percorso file o di una stringa Base64 nel formato PEM del certificato;
        \item \textbf{chiave-client}: contiene la chiave che ha firmato il certificato client;
        \item \textbf{token}: contiene un token per questo utente quando non è presente alcun certificato;
        \end{itemize}
    \item La sezione contexts specifica una combinazione di un utente e un cluster. \\Inoltre definisce  uno spazio dei nomi predefinito per questa coppia. Il nome del contesto è arbitrario, ma l'utente e il cluster devono essere predefiniti all'interno del \textbf{kubeconfig} file.
\end{itemize}
\subsubsection{Configurazione l'accesso a più cluster}
...
\end{lstlisting}
Supponiamo di avere due cluster, uno per il lavoro di sviluppo e uno per il lavoro di test. I tuoi sviluppatori front-end lavorano in uno spazio dei nomi chiamato frontende i tuoi sviluppatori di storage lavorano in uno spazio dei nomi chiamato storage. Nel tuo testcluster, gli sviluppatori lavorano nello spazio dei nomi predefinito o creano spazi dei nomi ausiliari come meglio credono. L'accesso al cluster di sviluppo richiede l'autenticazione tramite certificato. L'accesso al cluster di test richiede l'autenticazione tramite nome utente e password.\\
Passi da seguire:
\begin{enumerate}
    \item Crea una directory denominata \textbf{config-exercise}, con all'interno un file denominato \textbf{config-demo} con il seguente contenuto
    \begin{lstlisting}
apiVersion: v1
kind: Config
preferences: {}
...
- context:
  name: exp-test
    \end{lstlisting}
    \item aggiungere i dettagli del cluster 
    \begin{lstlisting}
kubectl config --kubeconfig=config-demo set-cluster development --server=https://1.2.3.4 --certificate-authority=fake-ca-file
kubectl config --kubeconfig=config-demo set-cluster test --server=https://5.6.7.8 --insecure-skip-tls-verify
    \end{lstlisting}
\item aggiungere i dettagli degli utenti
\begin{lstlisting}
kubectl config --kubeconfig=config-demo set-credentials developer --client-certificate=fake-cert-file --client-key=fake-key-seefile
kubectl config --kubeconfig=config-demo set-credentials experimenter --username=exp --password=some-password
\end{lstlisting}
\item aggiungere i dettagli del contesto di configurazione
\begin{lstlisting}
kubectl config --kubeconfig=config-demo set-context dev-frontend --cluster=development --namespace=frontend --user=developer
kubectl config --kubeconfig=config-demo set-context dev-storage --cluster=development --namespace=storage --user=developer
kubectl config --kubeconfig=config-demo set-context exp-test --cluster=test --namespace=default --user=experimenter
\end{lstlisting}
\item per visualizzare il risultato finale
\begin{lstlisting}
...
- name: experimenter
  user:
    ...
    username: exp
\end{lstlisting}
Per applicare tale configirazione utilizzare il comando: 
\begin{lstlisting}
    kubectl -f nome_file.yml 
\end{lstlisting}
\subsection{Esempio di deployments con Apache Druid}

\begin{enumerate}
    \item Creazione del service \textbf{Zookeeper}
    \begin{lstlisting}
apiVersion: v1
kind: Service
metadata:
...
spec:
  ...
  ports:
    ...
    \end{lstlisting}
\pagebreak
\item definire la \textbf{ConfigMap} per \textbf{Apache Druid} 
\begin{lstlisting}
apiVersion: v1
kind: ConfigMap
metadata:
...
\end{lstlisting}
\item definizione dei nodi Historical
\begin{lstlisting}
apiVersion: apps/v1
kind: Deployment
metadata:
...
\end{lstlisting}
\item definizione del nodo \textbf{Broker}
\begin{lstlisting}
apiVersion: apps/v1
kind: Deployment
metadata:
          ...
          configMap:
            ...
            items:
              - key: druid-broker.runtime.properties
                path: runtime.properties
\end{lstlisting}
\item definizione del nodo \textbf{Coordinator}
\begin{lstlisting}
apiVersion: apps/v1
kind: Deployment
metadata:
          ...
          configMap:
            ...
            items:
              - key: druid-coordinator.runtime.properties
                path: runtime.properties
\end{lstlisting}
\item definizione del \textbf{service Druid}
\begin{lstlisting}
apiVersion: v1
kind: Service
metadata:
  ...
  ports:
    ...
    - name: druid-coordinator
      ...
      selector:
        app: druid-coordinator
\end{lstlisting}
\item definire il nodo \textbf{Postgres}
\begin{lstlisting}
apiVersion: apps/v1
kind: Deployment
metadata:
    ...
    spec:
      ...
      volumes:
        - name: postgres-data
          emptyDir: {}
\end{lstlisting}
\item definire il service \textbf{Postgres}
\begin{lstlisting}
apiVersion: v1
kind: Service
metadata:
...
spec:
  ...
  ports:
    - name: postgres
      ...
      targetPort: 5432
\end{lstlisting}
\item dopo aver configurato i precedenti file \textbf{YAML} eseguire i seguenti comandi (i nomi dei file sono arbitrari)
\begin{lstlisting}
...
\end{lstlisting}
\item per verificare lo stato del cluster, eseguire i seguenti comandi
\begin{lstlisting}
...
\bibliography{https://druid.apache.org/docs/latest/operations/high-availability.html}
...
\bibliography{https://kubernetes.io/docs/tasks/access-application-cluster/configure-access-multiple-clusters/}  
\end{document}
