...
SELECT LOOKUP(customer, 'Carlo Albero dalla Chiesa'), 
SUM(cost)
FROM sales
GROUP BY 1
\end{lstlisting}
...
\end{itemize}
\pagebreak
\subsection{Creazione di un generatore di dati ed effettui una injection in Apache Druid tramite Apache Kafka}
...
\centering
 & \centering Apache Druid [s] & \centering PostgreSQL [s] \\\hline 
  ...
  \centering  query5 & \centering  & \centering  \\ \hline
  \centering  query6 & \centering  & \centering  \\ \hline
  \end{tblr}
\end{center}
